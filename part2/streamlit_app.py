...
if tab == "📊 Local Sales":
    ...
    df_monthly_sales = load_data("outputs/monthly_sales_by_category")
    st.altair_chart(
    ...
    df_top_sku = load_data("outputs/top_product_sku_by_amount")
    st.dataframe(df_top_sku)

    st.subheader("Sales by Fulfilment Type")
    df_fulfilment = load_data("outputs/sales_by_fulfilment")
    ...
    df_int_monthly = load_data("outputs/monthly_sales_by_customer")
    ...
    df_top_intl = load_data("outputs/top_intl_product_sku_by_amount")
    ...
    df_price = load_data("outputs/pricing_across_platforms")
    st.dataframe(df_price)

    st.subheader("Max Price Difference Per SKU")
    df_diff = load_data("outputs/max_price_diff_across_platforms")
    st.dataframe(df_diff)

    st.subheader("Average Price Per SKU")
    df_avg = load_data("outputs/avg_price_per_sku")
    st.dataframe(df_avg)
